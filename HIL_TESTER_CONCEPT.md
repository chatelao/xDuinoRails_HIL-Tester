...
## 6. Logic Analyzer and Signal Verification

A key feature of this HIL tester is its ability to use one of the Raspberry Pi Picos as a simple logic analyzer. By flashing the Pico with specialized firmware, it can capture digital signals and send the data back to the Raspberry Pi 3 host for analysis using the `sigrok-cli` tool.

This capability is ideal for verifying the correctness of signals generated by other SUTs, such as PWM, UART, SPI, or I2C.

### 6.1. Firmware

*   **Signal Capture Firmware:** We use the open-source **μLA (Micro Logic Analyzer)** firmware, which turns a Pico into a SUMP/OLS-compatible logic analyzer. A pre-compiled `.uf2` binary of this firmware is stored in the `firmware/` directory of this repository.
*   **Signal Generation Firmware:** To test the logic analyzer, a simple PWM signal generator firmware (`pico_pwm_generator`) is provided in the `test/pico_pwm_generator` directory.

### 6.2. Test Workflow

The logic analyzer test (`--test logic_analyzer`) automates the following steps:

1.  **Build:** Compiles the `pico_pwm_generator` firmware.
2.  **Flash SUT1:** Flashes the `pico_pwm_generator` firmware onto the first Pico (SUT1), which will act as the signal source.
3.  **Flash SUT2:** Flashes the `ula.uf2` logic analyzer firmware onto the second Pico (SUT2). This is done by first resetting the Pico into bootloader mode via OpenOCD, and then flashing the `.uf2` file with `picotool`.
4.  **Capture:** The host runs `sigrok-cli` to capture the signal from SUT2.
5.  **Analyze:** The Python test script parses the captured data, calculates the frequency of the signal, and compares it to the expected value to determine if the test has passed or failed.

### 6.3. Hardware Connections for the Logic Analyzer Test

To run the logic analyzer test, you must connect the signal output of SUT1 to a capture input on SUT2.

*   **SUT1 (PWM Generator):**
    *   **GPIO 16:** PWM Output
*   **SUT2 (Logic Analyzer):**
    *   **GPIO 0:** Logic Analyzer Input (Channel 0)

You will need to connect a jumper wire between **GPIO 16 on the first Pico** and **GPIO 0 on the second Pico**.

### 6.4. Running the Test

To execute the logic analyzer test, run the following command on the Raspberry Pi 3 host:

```bash
python3 script/run_hil_test.py --test logic_analyzer
```
